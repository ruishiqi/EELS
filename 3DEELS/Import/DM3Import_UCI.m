% DM3Import.m
% Data position slightly changed for UCI data
% ----------------
% Robert A. McLeod
% 09 May 2012
...
function [dm3struct] = DM3Import( dm3filename )
    ...
    for index = 1:numel(datatypeloc)
        ...
        % not 
        datatype(index) = uint8(metadm3file(datatypeloc(index)+15));
    end
    ...
    if( spectracount == 0 ) % IMAGES
            calibrateloc_curr = calibrateloc(1);
...
%                     'units', char( readTagData( unitloc(1) ).' ) );    
            ...
            dm3struct.xaxis = struct( 'scale', readTagData( scaleloc(1) ), ...
                            'origin', readTagData( originloc(1) ), ...
                            'units', char( readTagData( unitloc(1) ).' ) );

            % Images should have a yaxis
            if( ydim ~= 1 )
                dm3struct.yaxis = struct( 'scale', readTagData( scaleloc(2) ), ...
